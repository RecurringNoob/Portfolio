...
export const Terminal = ({ onClose }) => {
  ...
  const commands = {
    ...
    skills: () => {
      ...
      return [
        ...
        '',
        ...techList.map((tech, i) => `  ${(i + 1).toString().padStart(2)}. ${tech}`),
        '',
    ...
    work: () => {
      ...
      Object.values(allProfiles).forEach((profile) => {
        const workSection = profile.sections.find(
          (s) => s.title.toLowerCase().includes('work') || s.title.toLowerCase().includes('experience')
        );
    ...
    education: () => {
      ...
      Object.values(allProfiles).forEach((profile) => {
        const eduSection = profile.sections.find((s) => s.title.toLowerCase().includes('education'));
    ...
    resume: () => {
      ...
      Object.entries(allProfiles).forEach(([, profile]) => {
        ...
        profile.sections.forEach((section) => {
          output.push(`--- ${section.title} ---`);
          section.projects.forEach((project) => {
            output.push(`• ${project.title}`, `  ${project.year} | ${project.category}`);
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-50 flex flex-col p-6 font-mono selection:bg-green-500 selection:text-black">
      <button
        onClick={onClose}
